...
Max_iter = 10;
omega =0.5;
y0 = zeros(mm*(N+1),1); %vector to store the initial approximation
% populatiing initial value of y*
for i = 1:N
   y0(mm*(i-1)+1) = 1;
   y0(mm*(i-1)+2) = 0;
end
...
while (~Done)
    for i = 1:N
        ...
        Tip1 = yip1(1); %change
        qip1 = yip1(2); %change
        Aip1 = [-omega*qip1*Tip1^(omega-1), -Tip1^(omega-1); %change
            0, 0];
        Qip1 = [-qip1*Tip1^omega; sin(2*pi*xip1)];% change
        fip1 = Qip1-Aip1*yip1;
